...
/* It demonstrates Overall scrollable JPanel. It is edited from MyMenuOrder.java 
 * This is an interactive version of MyMenu of ReferenceProgram of Chapter8
 * It shows how to use radio buttons(JButton), check boxes(JCheckBox), scrollable text area(JTextArea)
 * list(JList), Menu and Menu Item as well.
 */
import java.awt.*;
...
public class  ScrollableGUI  extends JFrame implements ActionListener, ListSelectionListener
{
  ...
  JScrollPane scrollText = new JScrollPane(jtxtMenu);//JScrollPane object is to be added to the panel
  ...
  // Create a group for Entrees, each time only one radio button can be chosen
  // each radio button still needs to be added to panel for display 
  ...
  // Create a group for Hot Drinks, each time only one radio button can be chosen
  // each radio button still needs to be added to panel for display 
  ...
  p1.add(scrollText, BorderLayout.NORTH);
...
*/
  //make the overall display scrollable
  JPanel overallPanel = new JPanel();
  overallPanel.setLayout( new BoxLayout(overallPanel, BoxLayout.Y_AXIS ));//important 
  JScrollPane overallScroll=new JScrollPane(overallPanel);
  
  overallPanel.add(scrollText);
  overallPanel.add(pA);
  overallPanel.add(pB);
  overallPanel.add(pC);
  
  add(overallScroll);//add ScrollPane object to the frame
  //done for overall display scrollable 
  
  //below for menu and menu item display
    // add a small menu
  ...
  {
	ScrollableGUI frame = new ScrollableGUI();
